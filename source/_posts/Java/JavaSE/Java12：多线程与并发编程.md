...
* 原理不同：wait()方法释放cpu给其他线程，自己让出资源进入等待池等待；sleep占用cpu，不让出资源
...
* 使用区域不同：wait()方法只能在同步方法或同步代码块中调用，而sleep()能在任何地方调用
* wait()方法属于Object类,sleep()属于Thread类；sleep()必须指定时间，wait()可以指定时间也可以不指定
